...
class FrmMain( QMainWindow, ILegoViewModelObserver ):
    ...
    def __view_edge_details( self, s: Optional[ LegoEdge ] ):
        ...
        details.append( "{}[{}:{}" + E.format( s.source.sequence, s.source.start, s.source.end ) )
        details.append( L )
        details.append( S + "DESTINATION" + E )
        details.append( "{}[{}:{}" + E.format( s.destination.sequence, s.destination.start, s.destination.end ) )
        details.append( L )
