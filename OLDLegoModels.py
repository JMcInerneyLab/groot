...
class LegoCut:
    """
    A cut through a sequence (separating the two sides).
    Cuts can be combined, so cuts retain information on where they came from.
    """
    ...
    def merge_into_cut( self, b: "LegoCut" ):
        ...
        # Our replacement gets our positions and subsequences
        b.positions += self.positions
        b.subsequences += self.subsequences
        b.update( )
...
class LegoSequence:
    ...
    def get_subsequence( self, target: LegoBlastTarget ) -> LegoSubsequence:
        ...
        if not result in start_cut.subsequences:
            start_cut.subsequences.append(result)
        
        if not result in end_cut.subsequences:
            end_cut.subsequences.append(result)
