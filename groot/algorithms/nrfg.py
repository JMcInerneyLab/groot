...
from mgraph import MEdge, MGraph, MNode, analysing, Split, importing
...
def create_nrfg( model: LegoModel, cutoff: float, clean: bool, no_super: bool ) -> None:
    ...
    :param no_super:    Do not drop supersets.
...
def __nrfg_1_collect_splits( model ) -> Tuple[Set[Split],
    ...
    for component in model.components:
        ...
        component_splits = analysing.split_graph( tree,
                                                  filter = lambda x: isinstance( x.data, ILeaf ) )
...
def __nrfg_7_check_graph( nrfg: MGraph ):
    if len( nrfg.nodes ) == 0:
        raise ValueError( "The NRFG is bad. It doesn't contain any edges." )
    ...
    if len( ccs ) != 1:
        raise ValueError( "The NRFG is bad. It contains more than one connected component. It contains {}.".format( len( ccs ) ) )
...
def __debug_splits( graph: MGraph, query: Iterable[Split] ) -> Set[Split]:
    ...
    for edge in graph.edges:
        ...
        left: FrozenSet[ILeaf] = frozenset( lego_graph.get_ileaf_data( edge.follow_left() ) )
        right: FrozenSet[ILeaf] = frozenset( lego_graph.get_ileaf_data( edge.follow_right() ) )
